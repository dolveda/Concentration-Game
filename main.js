...
// 4B) If the next card selected does not match the previously turned card, both cards will revert to the "back" position and the player's current turn will end. The Turn indicator will decrease by 1, up to 3 turns total.
...
gameBoardArr.forEach((card, idx) => {
    card.addEventListener('click', function() {
        ...
        function checkForMatch() {
            if(userPicks[0] === userPicks[1]) {
                ...
                function decreaseTurns()  {
                    if(turnsLeft >= 1) {
                        turnsLeft--;
                        turnNumber.innerHTML = turnsLeft;
                    }
                }
                decreaseTurns();                
            }
        ...
        if(turnsLeft < 1) {
            ...
            matchIndicator.innerHTML = 'Out of Turns! Amapatoa Occeppa! (Play Again!)';
        }

        if(matchedCards.length === (cardChoices.length/2)) {
            matchIndicator.innerHTML = 'Cualtitoc! Tatlani!(Great job! You Win!)';
            playAgainBtn.style.visibility = 'visible';
        }
        };
    })
